...
#' interactively in an R shiny application. Depending on whether the parameter k
#' was specified in the calculation of the AOA, the user can either explore the `
#' AOA` and `DI` layer or additionally explore the `LPD` and a reactive k-dependent
#'  `AOA` layer.
#' The functionality is very suitable for a first visual interpretation of the
#' Area of Applicability.
#' @param aoa An R object of class `aoa`, generated by the aoa function. see \code{\link{aoa}}
#'
...
#' `Inside AOA` (1) or `Outside AOA`(0). The comparison is done for the nearest neighbor
#' of the training points in the predictor space. This can be seen in the AOA layer.
#' The LPD (Quantitative Density) layer indicates the total number of training data points
#' for which the locations DI is smaller than the threshold, i.e. on how many similar training points the model was trained.
#' The fourth layer is based on the LPD layer and can be seen as a k-dependet AOA. This layer extends the AOA
#' by a minimum number k of training data points for which the threshold condition must hold in order to be classified as `Indside AOA (1)`.
#' the location was inside the threshold in the DI
#' See "" for the full documentation of the methodology.
...
#'  \item{LPD}{SpatRaster, stars object or data frame. Quantitative Density of newdata. AOA has values 0 (outside AOA) and 1 (inside AOA)}
#'
#' @author
#' Fabian Schumacher
...
exploreAOA <- function(aoa) {
  ...
  xmark <- makeIcon("https://clipart-library.com/images/gie5B478T.png", iconWidth = 18, iconHeight = 18)
